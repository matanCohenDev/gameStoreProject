...
const updateProductFormBtn = document.getElementById(
  "UpdateProductBtnComplete"
);
...
function clearTables() {
  ...
  if (summaryTableBody) summaryTableBody.innerHTML = ""; // מנקה את תוכן הטבלה הקיים
  ...
  fetch("/api/users/getUsers")
    ...
    .then((data) => {
      data.forEach((user) => {
        ...
        if (user.roleLevel == 1) var role = "User";
        else if (user.roleLevel == 2) var role = "Creator";
        else if (user.roleLevel == 3) var role = "Admin";
        tr.innerHTML = `
                    <td>${user.username}</td>
                    <td>${user.email}</td>
                    <td>${role}</td>
    ...
    .then((data) => {
      ...
      data.forEach((categorySummary) => {
        ...
        tr.innerHTML = `
                ...
                `;
        if (summaryTableBody) summaryTableBody.appendChild(tr); // מוסיף את השורה לטבלה
      });
    })
    ...
    .then((data) => {
      data.forEach((order) => {
        ...
        const productNames = order.products
          ...
          .join(", ");
        const productQuantity = order.products
  ...
  try {
    ...
    if (response.ok) {
      ...
      const fbResponse = await fetch(
        "https://graph.facebook.com/v21.0/459877840548618/feed",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            message: `New Game Just arrived to GAME STORE: ${productName}. Check it out now!`,
            access_token:
              "EAASgcscfZA8YBO8XyuJWJjUr0d27aumKLNbxXhYiFW9qVr3qZAc2iuSyDEbN8ECFdZBdhjSUmtPZAY4z2H5Sa0FlgdNFNG53pElxetZB1tBMvZCZAzFCiJaYs13C4dIixsJBZCiUZCswAZAxxYXOZAe1wClkvXNsZAodCJYPsayEyZBQDZBN8HE19er9UZBPm7W4j9ZCCSH7",
          }),
        }
      );

      if (fbResponse.ok) {
        console.log("Post created on Facebook!");
      } else {
        console.error("Error posting to Facebook:", await fbResponse.json());
      }
    } else {
    ...
    for (const user of data) {
      if (user.username !== "admin") {
        ...
        userDiv.addEventListener("click", () => {
          if (chatWith !== null) chatWith.classList.remove("currentUser");
